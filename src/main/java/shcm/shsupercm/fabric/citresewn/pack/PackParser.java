                ...
                for (String root : ROOTS) {
                    ...
                    try {
                        ...
                        if (citPropertiesSupplier != null)
                            globalProperties.load(pack.packId(), identifier, citPropertiesSupplier.get());
                    } catch (FileNotFoundException ignored) {
                    } catch (Exception e) {
                        CITResewn.logErrorLoading("Errored while loading global properties: " + identifier + " from " + pack.packId());
                        e.printStackTrace();
                    }
                }
        return globalProperties;
    ...
    public static List<CIT<?>> parseCITs(ResourceManager resourceManager) {
        ...
        for (String root : ROOTS) {
            for (Map.Entry<Identifier, Resource> entry : resourceManager.listResources(root + "/cit", s -> s.getPath().endsWith(".properties")).entrySet()) {
                ...
                try {
                    cits.add(parseCIT(PropertyGroup.tryParseGroup(packName = entry.getValue().source().packId(), entry.getKey(), entry.getValue().open()), resourceManager));
