...
public final class CITRegistry { private CITRegistry(){}
    ...
    public static void registerAll() {
        ...
        for (var entrypointContainer : FabricLoader.getInstance().getEntrypointContainers(CITConditionContainer.ENTRYPOINT, CITConditionContainer.class)) {
            // Rule files distributed for the original CIT Resewn always assume the "citresewn" namespace
            // (implicitly or explicitly), regardless of which mod id actually registers the entrypoint on
            // this fork's side. Force it so existing packs keep working unmodified.
            String namespace = "citresewn";
        ...
        for (var entrypointContainer : FabricLoader.getInstance().getEntrypointContainers(CITTypeContainer.ENTRYPOINT, CITTypeContainer.class)) {
            ...
            final Identifier id = Identifier.fromNamespaceAndPath(namespace, entrypointContainer.getEntrypoint().id);
