...
public class CIT<T extends CITType> {
    ...
    public boolean test(CITContext context) {
        try {
            ...
            if (shcm.shsupercm.fabric.citresewn.CITResewn.DEBUG_LOG_TEST_EXCEPTIONS.add(propertiesIdentifier.toString() + ":" + context.stack.getItem()))
                shcm.shsupercm.fabric.citresewn.CITResewn.LOG.warn("[DEBUG] CIT.test() threw for " + propertiesIdentifier + " against " + context.stack.getItem(), e);
            return false;
        }
    }
}
