...
import shcm.shsupercm.fabric.citresewn.CITReloadListener;
...
public class ActiveCITs { private ActiveCITs() {}
    ...
    public static void load(ResourceManager resourceManager, ProfilerFiller profiler) {
        ...
        if (!cits.isEmpty()) {
            ...
            for (Map.Entry<Class<? extends CITType>, List<CIT<?>>> entry : active.cits.entrySet())
                perType.put(entry.getKey().getSimpleName(), entry.getValue().size());
            shcm.shsupercm.fabric.citresewn.CITResewn.info("[DEBUG] Loaded " + cits.size() + " CITs total: " + perType);
        }
    }
}
